...
func AfterTableAddColumn(ctx *sql.Context, runner sql.StatementRunner, nodeInterface sql.Node) error {
	...
	if !ok {
		return errors.Newf("ADD COLUMN post-hook expected `*plan.AddColumn` but received `%T`", nodeInterface)
	}
	...
	for _, otherTableName := range allTableNames {
		...
		for _, otherCol := range otherTableSch.GetAllCols().GetColumns() {
			...
			updateStr := fmt.Sprintf(`UPDATE "%s"."%s" SET "%s" = ROW(%s)::"%s"."%s" WHERE "%s" IS NOT NULL;`,
			...
			// We run the UPDATE as though it were interpreted since we're running a new statement inside the original
