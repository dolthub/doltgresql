...
func nodeGrant(node *tree.Grant) (vitess.Statement, error) {
	if node == nil {
		return nil, nil
	}
	...
	case privilege.Table:
		...
		grantTable = &pgnodes.GrantTable{
			Privileges:         convertPrivilegeKinds(auth.PrivilegeObject_TABLE, node.Privileges),
			Tables:             tables,
...
func convertPrivilegeKinds(object auth.PrivilegeObject, kinds []privilege.Kind) []auth.Privilege {
	privileges := make([]auth.Privilege, len(kinds))
	// TODO: check that each privilege is valid for the given object
	for i, kind := range kinds {
		switch kind {
		case privilege.ALL:
			// If we encounter ALL, then we know to return all privileges for this object
			return object.AllPrivileges()
		...
		default:
			...
			panic(fmt.Errorf("unknown privilege kind: %v", kind))
		}
	}
	return privileges
}
