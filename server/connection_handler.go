...
func (h *ConnectionHandler) handledWorkbenchCommands(statement string) (bool, error) {
	...
	if strings.HasPrefix(statement, `SELECT columns.*, pg_catalog.col_description(('"' || table_catalog || '"."' || table_schema || '"."' || table_name || '"')::regclass::oid, ordinal_position) AS description, ('"' || "udt_schema" || '"."' || "udt_name" || '"')::"regtype" AS "regtype", pg_catalog.format_type("col_attr"."atttypid", "col_attr"."atttypmod") AS "format_type" FROM "information_schema"."columns" LEFT JOIN "pg_catalog"."pg_attribute" AS "col_attr" ON "col_attr"."attname" = "columns"."column_name" AND "col_attr"."attrelid" = ( SELECT "cls"."oid" FROM "pg_catalog"."pg_class" AS "cls" LEFT JOIN "pg_catalog"."pg_namespace" AS "ns" ON "ns"."oid" = "cls"."relnamespace" WHERE "cls"."relname" = "columns"."table_name" AND "ns"."nspname" = "columns"."table_schema" ) WHERE ("table_schema" =`) {
		// re := regexp.MustCompile(`\(\s*"?table_schema"?\s*=\s*'([^']*)'\s*AND\s*"?table_name"?\s*=\s*'([^']*)'\s*\)`)

		// Find the matches
		// Convert the query to lowercase to find the last occurrence of "where"
		lastWhereIndex := strings.LastIndex(lower, " where ")
		if lastWhereIndex == -1 {
			return true, h.query(ConvertedQuery{
				String:       `SELECT '' FROM dual LIMIT 0;`,
				StatementTag: "SELECT",
		...
		// Extract everything after the last "WHERE"
		lastWhereClause := statement[lastWhereIndex:] // +7 to skip the "WHERE " itself

		query, err := h.convertQuery(fmt.Sprintf(`SELECT columns.*, '' AS description, ('"' || "udt_schema" || '"."' || "udt_name" || '"')::"regtype" AS "regtype", '' AS "format_type" FROM "information_schema"."columns" %s`, lastWhereClause))
		if err != nil {
			return false, err
		}
		return true, h.query(query)
