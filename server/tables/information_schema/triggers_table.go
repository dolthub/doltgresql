...
func triggersRowIter(ctx *sql.Context, catalog sql.Catalog) (sql.RowIter, error) {
	...
	for _, t := range trigs {
		...
		var condition any
		if matches := triggers.CreateTriggerWhenCapture.FindStringSubmatch(t.Definition); len(matches) == 2 {
			condition = "(" + matches[1] + ")"
		}
		var oldTable any
...
func triggerTiming(timing triggers.TriggerTiming) string {
	...
	case triggers.TriggerTiming_After:
		return "AFTER"
	default:
		//TODO: cover this once CREATE TRIGGER supports INSTEAD OF
		return "INSTEAD OF"
	}
}

// triggerEventManipulation returns the event_manipulation value for the given event type. TRUNCATE triggers are not
// listed by information_schema.triggers, so it returns an empty string for them.
