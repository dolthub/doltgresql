...
// initToRegtype registers the functions to the catalog.
func initToRegtype() {
	framework.RegisterFunction(to_regtype_text)
}

// to_regtype_text represents the PostgreSQL function of the same name, taking the same parameters.
var to_regtype_text = framework.Function1{
	Name:               "to_regtype",
	...
	Callable: func(ctx *sql.Context, _ [2]pgtypes.DoltgresType, val1 any) (any, error) {
		...
		oid, err := framework.IoInput(ctx, pgtypes.Regtype, val1.(string))
		if err != nil {
