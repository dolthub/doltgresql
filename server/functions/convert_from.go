...
var convert_from_bytea_name = framework.Function2{
	...
	Callable: func(ctx *sql.Context, _ [3]*pgtypes.DoltgresType, val1, val2 any) (any, error) {
		...
		if invalid := bytes.IndexRune(converted, utf8.RuneError); invalid >= 0 {
			validPrefix, _ := source.encoder.NewEncoder().Bytes(converted[:invalid])
			return nil, pgerror.WithCandidateCode(fmt.Errorf(`invalid byte sequence for encoding "%s": 0x%02x`,
				source.name, input[min(len(validPrefix), len(input)-1)]), pgcode.CharacterNotInRepertoire)
		}
