...
func (c *CreateType) RowIter(ctx *sql.Context, r sql.Row) (sql.RowIter, error) {
	...
	case types.TypeType_Pseudo:
		newType = types.NewShellType(ctx, id.NewInternalType(c.SchemaName, c.Name))
	case types.TypeType_Enum:
		typeID := id.NewInternalType(c.SchemaName, c.Name)
		arrayID := id.NewInternalType(c.SchemaName, "_"+c.Name)
		...
		for i, l := range c.Labels {
			...
			labelID := id.NewInternalEnumLabel(typeID, l)
