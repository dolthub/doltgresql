...
func serializeTypeArray(ctx *sql.Context, t *DoltgresType, val any) ([]byte, error) {
	...
	if t.serializedVersion == 0 && len(dims) > 1 {
		return nil, errors.Errorf("multidimensional arrays are not supported by the column's type version, alter the column's type to upgrade it")
	}
...
func deserializeArray(ctx *sql.Context, data []byte, baseType *DoltgresType) ([]any, error) {
	...
	return ReshapeArray(output, dims), nil
}
...
func flattenArray(vals []any) []any {
	...
	for _, subArray := range vals {
		flattened = append(flattened, flattenArray(subArray.([]any))...)
	}
	return flattened
}

// ReshapeArray nests the given row-major elements into an array value with the given dimensions.
func ReshapeArray(flattened []any, dims []int32) []any {
	...
	for i := range vals {
		vals[i] = ReshapeArray(flattened[i*subArrayLength:(i+1)*subArrayLength], dims[1:])
	}
	return vals
}
...
func ValidateAccumulatedArrays(vals []any) error {
	...
	for i, val := range vals {
		if val == nil {
			return errors.Errorf("cannot accumulate null arrays")
		}
		dims := arrayDims(val.([]any))
		if i == 0 {
			if len(dims) == 0 {
				return errors.Errorf("cannot accumulate empty arrays")
			}
			firstDims = dims
		} else if !slices.Equal(dims, firstDims) {
			return errors.Errorf("cannot accumulate arrays of different dimensionality")
		}
	}
	return nil
}
