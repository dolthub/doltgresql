...
func anySubqueryWithChildren(anyExpr *AnyExpr, sub *plan.Subquery) (sql.Expression, error) {
	...
	subTypes := make([]pgtypes.DoltgresTypeInterface, len(schema))
	for i, col := range schema {
		dgType, ok := col.Type.(pgtypes.DoltgresTypeInterface)
		if !ok {
			return nil, fmt.Errorf("expected right child to be a DoltgresTypeInterface but got `%T`", sub)
		}
		subTypes[i] = dgType
	}
	...
	if leftType, ok := anyExpr.leftExpr.Type().(pgtypes.DoltgresTypeInterface); ok {
		...
		for i, rightType := range subTypes {
			...
			if compFuncs[i].Type().(pgtypes.DoltgresTypeInterface).BaseID() != pgtypes.DoltgresTypeBaseID_Bool {
...
func anyExpressionWithChildren(anyExpr *AnyExpr) (sql.Expression, error) {
	...
	if !ok {
		return nil, fmt.Errorf("expected right child to be a DoltgresTypeInterface but got `%T`", anyExpr.rightExpr)
	}
	...
	if leftType, ok := anyExpr.leftExpr.Type().(pgtypes.DoltgresTypeInterface); ok {
		...
		if compFunc.Type().(pgtypes.DoltgresTypeInterface).BaseID() != pgtypes.DoltgresTypeBaseID_Bool {
			// This should never happen, but this is just to be safe
			return nil, fmt.Errorf("%T: found equality comparison that does not return a bool", anyExpr)
		}
