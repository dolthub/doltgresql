...
// TypeID returns the id.Type matching the column on the given type.
func (expr *ColumnAccess) TypeID() id.Type {
	if expr.colTyp != nil {
		return expr.colTyp.ID
	}
	if expr.child == nil {
		return id.NullType
	}
	return expr.child.Type().(*pgtypes.DoltgresType).CompositeAttrs[expr.colNameIdx].TypeID
}
