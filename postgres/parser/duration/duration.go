...
func (d Duration) AsInt64() (int64, bool) {
	...
	mSecs, ok := utils.MulHalfPositiveWithOverflow(numMonthsInYear, nanosInMonth/nanosInSecond)
	if !ok {
		return 0, ok
	}
	dSecs, ok := utils.MulHalfPositiveWithOverflow(d.Days, nanosInDay/nanosInSecond)
	if !ok {
		return 0, ok
	}
	if dSecs, ok = utils.AddWithOverflow(mSecs, dSecs); !ok {
		return 0, ok
	}
	if dSecs, ok = utils.AddWithOverflow(dSecs, int64(ySecs)); !ok {
		return 0, ok
	}
	return utils.AddWithOverflow(dSecs, d.nanos/nanosInSecond)
}
