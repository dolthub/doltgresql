...
import (
	...
	"github.com/dolthub/doltgresql/postgres/parser/hlc"
	"github.com/dolthub/doltgresql/postgres/parser/json"
	"github.com/dolthub/doltgresql/postgres/parser/kv"
...
var UnaryOps = unaryOpFixups(map[UnaryOperator]unaryOpOverload{
	...
	UnaryComplement: {
		...
		&UnaryOp{
			...
			Fn: func(_ *EvalContext, d Datum) (Datum, error) {
				p := MustBeDBitArray(d)
				return &DBitArray{BitArray: utils.Not(p.BitArray)}, nil
			},
			Volatility: VolatilityImmutable,
		},
		&UnaryOp{
...
var BinOps = map[BinaryOperator]binOpOverload{
	Bitand: {
		...
		&BinOp{
			...
			Fn: func(_ *EvalContext, left Datum, right Datum) (Datum, error) {
				...
				return &DBitArray{
					BitArray: utils.And(lhs.BitArray, rhs.BitArray),
				}, nil
			},
			Volatility: VolatilityImmutable,
		},
	...
	Bitor: {
		...
		&BinOp{
			...
			Fn: func(_ *EvalContext, left Datum, right Datum) (Datum, error) {
				...
				return &DBitArray{
					BitArray: utils.Or(lhs.BitArray, rhs.BitArray),
				}, nil
			},
			Volatility: VolatilityImmutable,
		},
	...
	Bitxor: {
		...
		&BinOp{
			...
			Fn: func(_ *EvalContext, left Datum, right Datum) (Datum, error) {
				...
				return &DBitArray{
					BitArray: utils.Xor(lhs.BitArray, rhs.BitArray),
				}, nil
			},
			Volatility: VolatilityImmutable,
		},
	...
	Plus: {
		&BinOp{
			...
			Fn: func(_ *EvalContext, left Datum, right Datum) (Datum, error) {
				...
				r, ok := utils.AddWithOverflow(int64(a), int64(b))
				if !ok {
					return nil, ErrIntOutOfRange
				}
	...
	Minus: {
		&BinOp{
			...
			Fn: func(_ *EvalContext, left Datum, right Datum) (Datum, error) {
				a, b := MustBeDInt(left), MustBeDInt(right)
				r, ok := utils.SubWithOverflow(int64(a), int64(b))
				if !ok {
					return nil, ErrIntOutOfRange
				}
				return NewDInt(DInt(r)), nil
	...
	Concat: {
		...
		&BinOp{
			...
			Fn: func(_ *EvalContext, left Datum, right Datum) (Datum, error) {
				...
				rhs := MustBeDBitArray(right)
				return &DBitArray{
					BitArray: utils.Concat(lhs.BitArray, rhs.BitArray),
				}, nil
			},
			Volatility: VolatilityImmutable,
		},
...
func TimestampToInexactDTimestamp(ts hlc.Timestamp) *DTimestamp {
	return MustMakeDTimestamp(time.Unix(0, ts.WallTime).UTC(), time.Microsecond)
}
