...
func (expr *CastExpr) TypeCheck(
	...
	ok, volatility := isCastDeepValid(castFrom, exprType)
	if !ok {
...
func (expr *CoalesceExpr) TypeCheck(
	...
	if err != nil {
		return nil, decorateTypeCheckError(err, "incompatible %s expressions", expr.Name)
	}
