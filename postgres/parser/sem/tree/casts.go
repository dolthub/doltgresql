...
func PerformCast(ctx *EvalContext, d Datum, t *types.T) (Datum, error) {
	...
	case types.BitFamily:
		...
		case *DCollatedString:
			res, err := utils.Parse(v.Contents)
			if err != nil {
				return nil, err
			}
			if t.Width() > 0 {
	...
	case types.TimestampFamily:
		...
		case *DInt:
			return MakeDTimestamp(time.Unix(int64(*d), 0).UTC(), roundTo)
		case *DTimestamp:
	...
	case types.TimestampTZFamily:
		...
		case *DInt:
			return MakeDTimestampTZ(time.Unix(int64(*d), 0).UTC(), roundTo)
		case *DTimestampTZ:
			return d.Round(roundTo)
		}
