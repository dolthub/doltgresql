...
import (
	...
	"github.com/dolthub/doltgresql/postgres/parser/protoutil"
...
func EncodeBitArrayAscending(b []byte, d utils.BitArray) []byte {
...
func EncodeBitArrayDescending(b []byte, d utils.BitArray) []byte {
...
func DecodeBitArrayAscending(b []byte) ([]byte, utils.BitArray, error) {
	if PeekType(b) != BitArray {
		return nil, utils.BitArray{}, errors.Errorf("did not find marker %x", b)
	}
	b = b[1:]
	...
	if len(b) == 0 || b[0] != bitArrayDataTerminator {
		return b, utils.BitArray{}, errBitArrayTerminatorMissing
	}
	b = b[1:]
	...
	ba, err := utils.FromEncodingParts(words, lastVal)
	return b, ba, err
}
...
func DecodeBitArrayDescending(b []byte) ([]byte, utils.BitArray, error) {
...
// appends it to the supplied buffer, and returns the final buffer.
func EncodeBitArrayValue(appendTo []byte, colID uint32, d utils.BitArray) []byte {
...
// supplied buffer, and returns the final buffer.
func EncodeUntaggedBitArrayValue(appendTo []byte, d utils.BitArray) []byte {
	bitLen := d.BitLen()
...
func DecodeBoolValue(buf []byte) (remaining []byte, b bool, err error) {
	...
	default:
		return buf, false, fmt.Errorf("value type is not %v or %v: %v", True, False, typ)
	}
}
...
func DecodeBitArrayValue(b []byte) (remaining []byte, d utils.BitArray, err error) {
...
func DecodeUntaggedBitArrayValue(b []byte) (remaining []byte, d utils.BitArray, err error) {
	var bitLen uint64
	b, _, bitLen, err = DecodeNonsortingUvarint(b)
	if err != nil {
		return b, utils.BitArray{}, err
	}
	words, lastBitsUsed := utils.EncodingPartsForBitLen(uint(bitLen))
	for i := range words {
		var val uint64
	...
	ba, err := utils.FromEncodingParts(words, lastBitsUsed)
	return b, ba, err
}
...
func PeekValueLengthWithOffsetsAndType(b []byte, dataOffset int, typ Type) (length int, err error) {
	...
	case BitArray:
		...
		numWords, _ := utils.SizesForBitLen(uint(bitLen))
