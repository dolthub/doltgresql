...
func (ipAddr *IPAddr) FromBuffer(data []byte) ([]byte, error) {
	...
	ipAddr.Addr = Addr(utils.FromBytes(data[2:]))
...
func (ipAddr IPAddr) String() string {
	ip := net.IP(utils.Uint128(ipAddr.Addr).GetBytes())
...
func (ipAddr *IPAddr) Complement() IPAddr {
	...
	var familyMask utils.Uint128
	if newIPAddr.Family == IPv4family {
		familyMask = utils.Uint128{Hi: 0, Lo: IPv4mask}
	} else {
		familyMask = utils.Uint128{Hi: math.MaxUint64, Lo: math.MaxUint64}
	}
	newIPAddr.Addr = Addr(utils.Uint128(ipAddr.Addr).Xor(familyMask))
...
func (ip Addr) subIPAddrIPv4(o Addr) (int64, error) {
	return int64(utils.Uint128(ip).Lo&IPv4mask) - int64(utils.Uint128(o).Lo&IPv4mask), nil
}
