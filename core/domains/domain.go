...
func NewDomain(ctx *sql.Context, name string, typ types.DoltgresType, notNull bool, checks sql.CheckConstraints) *Domain {
...
// GetDomain returns the sequence with the given schema and name. Returns nil if the domain cannot be found.
...
func (pgs *DomainCollection) CreateDomain(schema string, domain *Domain) error {
	...
	if _, ok = nameMap[domain.Name]; ok {
		return fmt.Errorf(`relation "%s" already exists`, domain.Name)
	}
