...
func testSQLRacingAmend(t *testing.T) {
	t.Skip("Doltgres surfaces concurrent racing inserts as a 'duplicate primary key' error rather than the transaction-conflict error this test expects (behavioral difference from Dolt)")
	t.Parallel()
	ctx := context.Background()
	ports := newPorts(t)
	u, err := driver.NewDoltUser()
