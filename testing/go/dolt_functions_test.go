	...
	RunScripts(t, []ScriptTest{
		...
		},
		{
			...
			Assertions: []ScriptTestAssertion{
				...
				{
					Skip:  true, // TODO: ERROR: function 'dolt_diff' expected 3 arguments, 2 received
					Query: "SELECT diff_type, from_pk, to_pk FROM dolt_diff('HEAD', 'WORKING')",
					Expected: []sql.Row{
						{"added", nil, 1},
					},
				},
				...
				},
				{
					...
					Expected: []sql.Row{
						{"WORKING", "t1", nil, nil, nil, nil, 1, 1},
					},
				},
				{
					Query: "SELECT statement_order, table_name, diff_type, statement FROM dolt_patch('HEAD', 'WORKING')",
					Expected: []sql.Row{
						{Numeric("1"), "t1", "schema", "CREATE TABLE `t1` (\n  `pk` integer NOT NULL,\n  PRIMARY KEY (`pk`)\n) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_bin;"},
						{Numeric("2"), "t1", "data", "INSERT INTO `t1` (`pk`) VALUES (1);"},
					},
				},
				{
