...
func TestAuthTests(t *testing.T) {
	RunScripts(t, []ScriptTest{
		...
		{
			Name: `DROP ROLE removes inherited privileges`,
			SetUpScript: []string{
				`CREATE TABLE drop_role_test (v integer);`,
				`INSERT INTO drop_role_test VALUES (1);`,
				`CREATE ROLE dropped_group;`,
				`CREATE USER surviving_member PASSWORD 'password';`,
				`GRANT SELECT ON drop_role_test TO dropped_group;`,
				`GRANT dropped_group TO surviving_member;`,
			},
			Assertions: []ScriptTestAssertion{
				{
					Query:    `SELECT * FROM drop_role_test;`,
