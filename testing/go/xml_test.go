	...
	RunScripts(t, []ScriptTest{
		{
			...
			Assertions: []ScriptTestAssertion{
				...
				{
					Query:    "SELECT xpath('//x:b/text()', '<a xmlns:x=\"urn:x\"><x:b>1</x:b></a>'::xml, ARRAY[ARRAY['x','urn:x']]), xpath('//y:b/text()', '<a xmlns:x=\"urn:x\"><x:b>1</x:b></a>'::xml, ARRAY[ARRAY['y','urn:x']]);",
					Expected: []sql.Row{{"{1}", "{1}"}},
				},
				{
					Query:    "SELECT xpath('//x:b', '<a xmlns:x=\"urn:x\"><x:b>1</x:b></a>'::xml, ARRAY[ARRAY['x','urn:x']]), xpath('/d:a/d:b', '<a xmlns=\"urn:d\"><b>1</b></a>'::xml, ARRAY[ARRAY['d','urn:d']]);",
				...
				{
					Query:           "SELECT xpath('//x:b/text()', '<a xmlns:x=\"urn:x\"><x:b>1</x:b></a>'::xml, ARRAY[ARRAY['x',NULL]]);",
					ExpectedErr:     "neither namespace name nor URI may be null",
		...
		},
		{
			...
			Assertions: []ScriptTestAssertion{
				...
				{
					Query:    "SELECT xpath_exists('//x:b', '<a xmlns:x=\"urn:x\"><x:b>1</x:b></a>'::xml, ARRAY[ARRAY['x','urn:x']]), xpath_exists(NULL, '<a/>'::xml);",
		...
		},
		{
			...
			Assertions: []ScriptTestAssertion{
				...
				},
				{
					...
					ExpectedErr:     "namespace URI must not be null",
					ExpectedErrCode: "22004",
				},
				{
					Query:           "SELECT * FROM XMLTABLE('/r' PASSING '<r/>' COLUMNS n text PATH '');",
					ExpectedErr:     "column path filter must not be empty string",
					ExpectedErrCode: "22000",
				},
			},
		},
		{
			...
			SetUpScript: []string{
				...
				`INSERT INTO big_docs VALUES (1, ('<r>' || repeat('<i n="a">1</i>', 2000) || '</r>')::xml, '<r>' || repeat('<i n="b">2</i>', 2000) || '</r>'), (2, repeat('<i>x</i>', 3000)::xml, repeat('c', 20000));`,
			},
			Assertions: []ScriptTestAssertion{
				{
					Query:    "SELECT id, length(doc::text), md5(doc::text), length(src), md5(src) FROM big_docs ORDER BY id;",
					Expected: []sql.Row{{1, 28007, "5926b51458cff20722ee4a900d911e63", 28007, "ec757e5d3da6bcdbad50e56e8905a8a8"}, {2, 24000, "3910aa29146c00ddc042df788598acc3", 20000, "3fb3fedd0620db447a450f9b4cda4bdd"}},
				},
				{
					...
					Expected: []sql.Row{{2000, "2000", "a"}},
				},
				{
					Query:    `SELECT xpath_exists('/r/i[2000]', doc), xpath_exists('/r/i[2001]', doc), xmlexists('/r/i[@n="a"]' PASSING doc), xmlexists('/r/i[@n="b"]' PASSING BY REF doc) FROM big_docs WHERE id = 1;`,
				...
				},
				{
					...
					Expected: []sql.Row{{28045, "efecfb6b601cf31ddb3c6d02ae1b3112"}},
				},
				{
				...
				},
				{
					...
					Expected: []sql.Row{{1, 28011, "f"}, {2, 24000, "f"}},
				},
			},
		},
	})
