...
func TestLargeobject(t *testing.T) {
	t.Skip()
	_ = RunTests(t, RegressionFileName_largeobject)
}

func init() {
	RegisterRegressionFile(RegressionFile{
		RegressionFileName: RegressionFileName_largeobject,
...
INSERT INTO lotest_stash_values (loid) SELECT lo_import(:'filename');`,
			...
			},
			{
				...
				Results:   []sql.Row{{`AAA\011FBAAAA\011VVVVxx\0122513\01132\0111\0111\0113\01113\0111`}},
			},
			{
				Statement: `SELECT lo_tell(fd) FROM lotest_stash_values;`,
			...
			{
				Statement: `SELECT loread(fd, 36) FROM lotest_stash_values;`,
				Results:   []sql.Row{{`AAA\011FBAAAAabcdefghijklmnop1\0111\0113\01113\0111`}},
			},
			{
				Statement: `SELECT lo_close(fd) FROM lotest_stash_values;`,
				Results:   []sql.Row{{0}},
