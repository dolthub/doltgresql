...
func TestConcurrentSequences(t *testing.T) {
	const (
		dbName       = "doltgres_concurrency_repro"
	...
	sc, doltgresConfig := startServer(t, "", "")
...

	{
		...
		_, err = conn.Exec(ctx, fmt.Sprintf(
			`CREATE SEQUENCE "%s" AS integer START WITH 1 INCREMENT BY 2 NO MINVALUE NO MAXVALUE CACHE 1;`, sequenceName))
		require.NoError(t, err)
		_, err = conn.Exec(ctx, fmt.Sprintf(
			`CREATE TABLE "%s" (id INT NOT NULL PRIMARY KEY)`, tableName))
		require.NoError(t, err)
	}
	...
	for i := 1; i <= threadCount; i++ {
		...
		go func(i int) {
			...
			_, err := conn.Exec(ctx,
				fmt.Sprintf(`INSERT INTO "%s" (id) SELECT nextval($1)`, tableName),
				sequenceName)
			if err != nil {
				mu.Lock()
	...
	require.NoError(t, report.QueryRow(ctx, fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, tableName)).Scan(&rowCount))
	require.NoError(t, report.QueryRow(ctx, fmt.Sprintf(`SELECT COUNT(DISTINCT id) FROM "%s"`, tableName)).Scan(&distinctIds))
	require.NoError(t, report.QueryRow(ctx, fmt.Sprintf(`SELECT MAX(id) FROM "%s"`, tableName)).Scan(&maxID))
